...
export const SecondStepForm: React.FC<SecondStepFormType> = ({onCLose}) => {
    ...
    const submitHandler = async (values: FormikValues, setSubmitting: (status: boolean) => void) => {
        await dispatch(addPhoto(values.file) as unknown as AnyAction)
        setSubmitting(false)
        onCLose()
    }


    return <Box className={style.step2form}>
        ...
        <Formik
            ...
            {({isSubmitting, setFieldValue, resetForm}) => {
                ...
                return <Box className={style.buttons2}>
                    <Form>
                        ...
                        {
                            image && <Button variant="contained"
                                             color="secondary"
                                             onClick={clearHandler}>
                                Clear</Button>
                        }

                        {image ? <Button type="submit"
